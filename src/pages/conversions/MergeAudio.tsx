...
export default function MergeAudio() {
  ...
  const handleFinishUpload = () => {
    if (selectedFiles.length < 2) {
      toast.error("Please select at least 2 audio files to merge");
      return;
    }
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      const blob = new Blob([await selectedFiles[0].arrayBuffer()], { type: "audio/mp3" });
      downloadFile(blob, "merged.mp3");
  ...
  return (
    <>
      <Helmet>
        <title>Free Audio Merger Online | Combine Audio Files | ConvertAny</title>
        <meta name="description" content="Merge audio files online for free. Combine multiple MP3, WAV files into one." />
        <meta name="keywords" content="merge audio, combine audio, audio merger, join audio, online audio merge" />
      </Helmet>
                ...
                {isUploading ? (
                  <div className="space-y-4">
                    <div className="border-2 border-dashed border-primary/20 rounded-lg p-12 text-center hover:border-primary/40 transition-colors">
                      ...
                      <p className="text-lg mb-4">Upload audio files to merge</p>
                      <input
                        type="file"
                  ...
                  <div className="space-y-6">
                    <p className="font-semibold">Arrange Audio Order</p>
                    <div className="space-y-2">
                      {selectedFiles.map((file, index) => (
                        <div key={index} className="flex items-center gap-3 p-3 border rounded-lg bg-muted/20">
                    ...
                    <div className="flex gap-3">
                      <Button onClick={handleMerge} disabled={isMerging} className="flex-1 gap-2">
                        <Download className="h-4 w-4" />
                        {isMerging ? "Merging..." : "Merge & Download"}
                      </Button>
                      <Button variant="outline" onClick={() => { setSelectedFiles([]); setIsUploading(true); }}>
                        Start Over
                      </Button>
                    </div>
                  </div>
